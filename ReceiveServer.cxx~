...
int ReceiveTransform(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    // Meysam Nov 29, 2013:
    /*int delayTime = 100 * matrix[0][3];
    int data = 255;  // data we want to send to Arduino
    FILE *file;
    file = fopen("/dev/ttyACM0","w");  //Opening the device file
    for(int i = 0 ; i < delayTime ; i++)
    {
            fprintf(file,"%d",data); //Writing to the device file
            fprintf(file,"%c",','); 
     }
    fclose(file);*/
